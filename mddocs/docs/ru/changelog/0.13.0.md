...
- Все подключения к БД, открытые `JDBC.fetch(...)`, `JDBC.execute(...)` или `JDBC.check()`, немедленно закрываются после выполнения операторов. ([#334](https://github.com/MobileTeleSystems/onetl/pull/334))
