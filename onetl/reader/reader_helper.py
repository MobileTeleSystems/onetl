...
# TODO: разобрать этот понос, возможно выпилить
class BaseRemoteFile:
    ...
    # TODO: Это просто адовый ад, тут нужно все переписать
    def check(self, files: List[str], files_history: Optional = None, check_history: bool = True):
        """
        ...
        if check_history:
            self.check_history(self.remote_file_path, files_history)

        if self.remote_source_file_flag_download:
            self.check_pattern(self.remote_flag_file_path)
            if check_history:
                self.check_history(self.remote_flag_file_path, files_history)
    ...
    def check_history(self, res_file: str, files_history):
        if not self.lookup_history:
            return True
        elif res_file not in files_history.get_files():
            return True
