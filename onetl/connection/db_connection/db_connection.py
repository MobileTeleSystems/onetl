...
class DBConnection(ConnectionABC):
    ...
    # TODO: Create abstract class for engine. Engine uses pyhive session or Engine uses pyspark session
    ...
    def read_table(
        ...
        # TODO: Here will need to implement a hint in case of typos in writing parameters
        if not self.spark:
        ...
        log.debug(
            f'USER="{self.user}" '
            f'OPTIONS={options} DRIVER={options["properties"]["driver"]}',
        )
