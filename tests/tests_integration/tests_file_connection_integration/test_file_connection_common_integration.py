...
def test_file_connection_rename_file(file_connection_with_path_and_files, path_type):
    ...
    with file_connection as connection:
        connection.rename_file(
            source_file_path=path_type(os.fspath(remote_path / "exclude_dir/file_5.txt")),
            target_file_path=path_type(os.fspath(remote_path / "exclude_dir/file_55.txt")),
        )

    list_dir = file_connection.list_dir(remote_path / "exclude_dir/")
...
def test_file_connection_write_text_fail_on_bytes_input(file_connection_with_path):
    ...
    with pytest.raises(TypeError):
        file_connection.write_text(path=remote_path / "some_file_name.txt", content=b"bytes to text")


def test_file_connection_write_bytes_fail_on_text_input(file_connection_with_path):
    file_connection, remote_path = file_connection_with_path
    with pytest.raises(TypeError):
        file_connection.write_bytes(path=remote_path / "some_file_name.txt", content="text to bytes")
...
def test_file_connection_write_encoding(file_connection_with_path):
    ...
    assert file_connection.read_bytes(path=remote_path / "cp_1251_file") == "тестовый текст в  utf-8".encode(
        "cp1251",
    )
...
def test_file_connection_path_exists(file_connection_with_path_and_files, path_type):
    file_connection, remote_path, _ = file_connection_with_path_and_files
    assert file_connection.path_exists(path_type(os.fspath(remote_path / "exclude_dir/file_5.txt")))
    assert file_connection.path_exists(path_type(os.fspath(remote_path / "exclude_dir")))
...
def test_file_connection_is_dir(file_connection_with_path_and_files, path_type):
    file_connection, remote_path, _ = file_connection_with_path_and_files
    assert file_connection.is_dir(path_type(os.fspath(remote_path / "exclude_dir")))
    assert not file_connection.is_dir(path_type(os.fspath(remote_path / "exclude_dir/file_5.txt")))
...
def test_file_connection_is_file(file_connection_with_path_and_files, path_type):
    file_connection, remote_path, _ = file_connection_with_path_and_files
    assert file_connection.is_file(path_type(os.fspath(remote_path / "exclude_dir/file_5.txt")))
    assert not file_connection.is_file(path_type(os.fspath(remote_path / "exclude_dir")))
...
def test_file_connection_download_file(
    ...
    remote_file_path = remote_path / "news_parse_zp/2018_03_05_10_00_00/newsage-zp-2018_03_05_10_00_00.csv"
...
def test_file_connection_download_file_wrong_target_type(
    ...
    with pytest.raises(NotAFileError):
        file_connection.download_file(
            remote_file_path=remote_path / "exclude_dir/file_5.txt",
...
def test_file_connection_upload_file_wrong_source_type(file_connection, resource_path, source, exception):
    with pytest.raises(exception):
...
def test_file_connection_upload_replace_target(
    ...
    upload_result = file_connection.upload_file(
        local_file_path=path_type(file_path),
        remote_file_path=path_type(os.fspath(remote_path / "exclude_dir/file_5.txt")),
        replace=True,
    )
