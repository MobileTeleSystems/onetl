"""Integration tests for FileReader.

Test only that options generated by both FileReader and FileReader.Options are passed to Spark,
...
def test_file_reader_run_with_files(
    file_df_connection_with_path_and_files,
    file_df_dataframe,
    pass_source_path,
    caplog,
...
def test_file_reader_run_with_files_relative_and_source_path(
    file_df_connection_with_path_and_files,
    file_df_dataframe,
):
    file_df_connection, source_path, uploaded_files = file_df_connection_with_path_and_files
...
def test_file_reader_run_without_files_and_source_path(file_df_connection):
    reader = FileReader(
        connection=file_df_connection,
        format=CSV(),
    )
...
def test_file_reader_run_with_empty_files_input(
    file_df_connection_with_path_and_files,
    pass_source_path,
    file_df_schema,
):
...
def test_file_reader_run_with_empty_source_path(file_df_connection_with_path, file_df_schema):
    file_df_connection, source_path = file_df_connection_with_path
...
def test_file_reader_run_relative_path_without_source_path(file_df_connection):
    reader = FileReader(
        connection=file_df_connection,
        format=CSV(),
    )
...
def test_file_reader_run_absolute_path_not_match_source_path(file_df_connection_with_path_and_files):
    file_df_connection, source_path, _ = file_df_connection_with_path_and_files
    reader = FileReader(
        connection=file_df_connection,
        format=CSV(),
...
def test_file_reader_source_path_does_not_exist(file_df_connection, file_df_schema):
...
def test_file_reader_source_path_cannot_be_file(file_df_connection_with_path_and_files, file_df_schema):
    file_df_connection, source_path, _ = file_df_connection_with_path_and_files
...
def test_file_reader_run_no_files_and_no_schema(
...
def test_file_reader_run_recursive_false(spark, file_df_connection_with_path_and_files, file_df_schema):
    spark_version = ".".join(spark.version.split(".")[:2])
...
def test_file_reader_run_recursive_true(
    spark,
    file_df_connection_with_path_and_files,
    file_df_dataframe,
):
...
def test_file_reader_run_partitioned(
    file_df_connection_with_path_and_files,
...
def test_file_reader_run_partitioned_recursive(spark, file_df_connection_with_path_and_files, file_df_dataframe):
    spark_version = ".".join(spark.version.split(".")[:2])
    if spark_version < "3.2":
        pytest.skip("Option `recursive` is not supported on Spark 2")
