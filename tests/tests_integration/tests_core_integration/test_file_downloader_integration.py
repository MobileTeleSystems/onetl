...
def test_file_downloader_file_filter_exclude_dir(
    ...
    excluded = [
        remote_path / "exclude_dir/file_4.txt",
        remote_path / "exclude_dir/file_5.txt",
    ]
...
def test_file_downloader_file_filter_glob(file_connection_with_path_and_files, tmp_path_factory, caplog):
    ...
    excluded = [
        remote_path / "utf-8.txt",
        remote_path / "ascii.txt",
        remote_path / "exclude_dir/file_4.txt",
        remote_path / "exclude_dir/file_5.txt",
        remote_path / "news_parse_zp/exclude_dir/file_1.txt",
        remote_path / "news_parse_zp/exclude_dir/file_2.txt",
        remote_path / "news_parse_zp/exclude_dir/file_3.txt",
    ]
...
def test_file_downloader_run_with_empty_source_path(request, file_connection_with_path, tmp_path_factory):
    ...
    remote_path = PurePosixPath(f"/tmp/test_upload_{secrets.token_hex(5)}")
...
def test_file_downloader_limit_applied_after_filter(file_connection_with_path_and_files, tmp_path_factory):
    ...
    filtered = {
        local_path / file.relative_to(remote_path) for file in uploaded_files if os.fspath(file) not in excluded
    }
...
def test_file_downloader_detect_hwm_type_snap_batch_strategy(
...
def test_file_downloader_detect_hwm_type_inc_batch_strategy(
    file_connection_with_path,
    tmp_path_factory,
):
    file_connection, remote_path = file_connection_with_path
