...
def test_uploader_run_with_files(file_connection, test_files, run_path_type, path_type):
...
def test_uploader_run_with_local_path(file_connection, resource_path, path_type):
...
def test_uploader_run_missing_file(file_connection, test_files, caplog):
    target_path = PurePosixPath(f"/tmp/test_upload_{secrets.token_hex(5)}")
...
def test_uploader_run_delete_local(resource_path, test_files, file_connection, caplog):
    target_path = PurePosixPath(f"/tmp/test_upload_{secrets.token_hex(5)}")
...
def test_uploader_run_mode_error(request, file_connection, test_files, options):
    target_path = PurePosixPath(f"/tmp/test_upload_{secrets.token_hex(5)}")
...
def test_uploader_run_mode_ignore(request, file_connection, test_files, caplog):
    target_path = PurePosixPath(f"/tmp/test_upload_{secrets.token_hex(5)}")
...
def test_uploader_run_mode_overwrite(request, file_connection, test_files, caplog):
    target_path = PurePosixPath(f"/tmp/test_upload_{secrets.token_hex(5)}")
...
def test_uploader_run_local_path_does_not_exist(file_connection, tmp_path_factory):
    target_path = PurePosixPath(f"/tmp/test_upload_{secrets.token_hex(5)}")
...
def test_uploader_run_local_path_not_a_directory(file_connection):
    target_path = PurePosixPath(f"/tmp/test_upload_{secrets.token_hex(5)}")
...
def test_uploader_run_target_path_not_a_directory(request, file_connection, resource_path):
    target_path = PurePosixPath(f"/tmp/test_upload_{secrets.token_hex(5)}")
...
def test_uploader_run_input_is_not_file(file_connection, test_files):
    target_path = PurePosixPath(f"/tmp/test_upload_{secrets.token_hex(5)}")
...
def test_uploader_run_with_empty_files(file_connection, pass_local_path, tmp_path_factory):
    target_path = PurePosixPath(f"/tmp/test_upload_{secrets.token_hex(5)}")
...
def test_uploader_run_with_empty_local_path(file_connection, tmp_path_factory):
    target_path = PurePosixPath(f"/tmp/test_upload_{secrets.token_hex(5)}")
    local_path = tmp_path_factory.mktemp("local_path")
...
def test_uploader_without_files_and_without_local_path(file_connection):
    target_path = PurePosixPath(f"/tmp/test_upload_{secrets.token_hex(5)}")
...
def test_uploader_run_with_relative_files_and_local_path(file_connection, resource_path, caplog):
    target_path = PurePosixPath(f"/tmp/test_upload_{secrets.token_hex(5)}")
...
def test_uploader_run_with_absolute_files_and_local_path(file_connection, resource_path, caplog):
    target_path = PurePosixPath(f"/tmp/test_upload_{secrets.token_hex(5)}")
...
def test_uploader_run_absolute_path_not_match_local_path(file_connection, resource_path):
    target_path = PurePosixPath(f"/tmp/test_upload_{secrets.token_hex(5)}")
...
def test_uploader_run_relative_paths_without_local_path(file_connection):
    target_path = PurePosixPath(f"/tmp/test_upload_{secrets.token_hex(5)}")
