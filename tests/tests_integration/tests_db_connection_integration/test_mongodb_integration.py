...
def test_mongodb_snapshot_with_where(spark, processing, load_table_data):
    ...
    table_df1 = mongo.read_table(table=load_table_data.table, hint={"_id": 1}, where={"_id": {"$lt": 1000}})
    ...
    one_df = mongo.read_table(table=load_table_data.table, hint={"_id": 1}, where={"_id": {"$eq": 50}})

    assert one_df.count() == 1

    empty_df = mongo.read_table(table=load_table_data.table, hint={"_id": 1}, where={"_id": {"$gt": 1000}})
