...
class TestOnETLPostgresToHive:
    ...
    def test_append_mode(self, spark):
        with psycopg2.connect(environ["ONETL_PG_CONN"]) as postgres_conn:
            ...
            fiddle_stub.bulk_insert(
                "fiddle",
                "rdb_to_hive_append_mode",
                ["d_id", "d_name"],
                ((i + 1, "guy") for i in range(10 ** 3)),
            )
    ...
    def test_lower_upper_bound_filtering(self, spark):
        with psycopg2.connect(environ["ONETL_PG_CONN"]) as postgres_conn:
            fiddle_stub = postgres_stubs.PostgresTableStub(postgres_conn)
            fiddle_stub.bulk_insert(
                "fiddle",
...
class TestOnETLHiveToPostgres:
    ...
    def test_snapshot_hive_to_postgres_with_custom_where(
        ...
        jdbc_options = {"batchsize": "100"}
        hive = Hive(spark=spark)
        postgres = Postgres(host="postgres", user="onetl", password="onetl", database="onetl", spark=spark)  # NOSONAR
