...
class TestHDFS:
    ...
    def test_hdsf_file_uploader_delete_source(self, make_test_files_copy, hdfs_server, path_type, run_path_type):

        hdfs = HDFS(host=hdfs_server.host, port=hdfs_server.port)

        target_path = path_type(f"/user/onetl/test_upload_{secrets.token_hex(5)}")
        uploader = FileUploader(connection=hdfs, target_path=target_path, delete_local=True)
    ...
    @pytest.mark.parametrize("path_type", [str, Path])
    def test_hdfs_file_downloader_delete_source(self, hdfs_server, source_path, resource_path, hdfs_files, path_type):
        hdfs = HDFS(host=hdfs_server.host, port=hdfs_server.port)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            files = downloader.run()
            local_files = [Path(temp_dir) / file.name for file in hdfs_files]
